...
const listingSchema = z.object({
  ...
  propertyType: z.enum(['APARTMENT', 'HOUSE', 'DUPLEX', 'STUDIO', 'OFFICE', 'SHOP', 'WAREHOUSE', 'LAND']),
...
const LAGOS_REGIONS_DATA = [
  { id: 'reg-001', name: 'Lagos Island', lgas: ['Lagos Island', 'Eti-Osa', 'Ibeju-Lekki'] },
  { id: 'reg-002', name: 'Lagos Mainland', lgas: ['Lagos Mainland', 'Surulere', 'Yaba', 'Mushin', 'Oshodi-Isolo'] },
  { id: 'reg-003', name: 'Ikeja', lgas: ['Ikeja', 'Kosofe', 'Shomolu', 'Agege', 'Ifako-Ijaiye'] },
  { id: 'reg-004', name: 'Ikorodu', lgas: ['Ikorodu'] },
  { id: 'reg-005', name: 'Badagry', lgas: ['Badagry', 'Ojo', 'Amuwo-Odofin', 'Ajeromi-Ifelodun', 'Apapa'] },
...
const AMENITIES = [
  'Swimming Pool', 'Gym', 'Security', '24/7 Power', 'Water Supply',
  'Parking Space', 'Serviced', 'Elevator', 'Air Conditioning', 'Balcony',
  'Garden', 'Boys Quarters', 'CCTV', 'Gated Community', 'Waterfront',
];
...
export function AddListingModal({ isOpen, onClose, onSuccess }: AddListingModalProps) {
  ...
  const availableLgas = LAGOS_REGIONS_DATA.find((r) => r.id === selectedRegion)?.lgas ?? [];
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-xl bg-surface-raised border border-sidebar-border shadow-2xl animate-fade-in">
        ...
        <div className="flex items-center justify-between p-6 border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Add Official Listing</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="p-6 max-h-[calc(90vh-200px)] overflow-y-auto">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    {errors.propertyType && (
                      <p className="text-sm text-danger">{errors.propertyType.message}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {errors.listingType && (
                      <p className="text-sm text-danger">{errors.listingType.message}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {selectedPropertyType && !['OFFICE', 'SHOP', 'WAREHOUSE', 'LAND'].includes(selectedPropertyType) && (
                    <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.bathrooms && (
                      <p className="text-sm text-danger">{errors.bathrooms.message}</p>
                    )}
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-300">Size (m²)</label>
                    <input
                      type="number"
                      min="1"
                      className="w-full rounded-lg border border-sidebar-border bg-surface py-3 px-4 text-white focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20"
                    ...
                    />
                    {errors.size && (
                      <p className="text-sm text-danger">{errors.size.message}</p>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {LAGOS_REGIONS_DATA.map((region) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-500 mt-1">{region.lgas.length} LGAs</p>
                      </button>
                    ))}
                  </div>
                  {errors.regionId && (
                    <p className="text-sm text-danger">{errors.regionId.message}</p>
                  )}
                </div>
                ...
                {selectedRegion && (
                  <div className="space-y-2">
                    ...
                    {errors.lgaId && (
                      <p className="text-sm text-danger">{errors.lgaId.message}</p>
                    )}
                  </div>
                )}
            ...
            {step === 3 && (
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-white">Amenities & Images</h3>
                ...
                {/* Image Upload */}
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-300">Property Images</label>
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    {images.length < 6 && (
                      <button
                        type="button"
                        onClick={() => setImages([...images, 'https://placehold.co/400x300'])}
                  ...
                  <p className="text-xs text-gray-500">Upload up to 6 images. First image will be the cover.</p>
                </div>

                {/* Summary */}
                <div className="p-4 rounded-lg bg-sidebar-hover">
                  <h4 className="text-sm font-medium text-gray-300 mb-2">Listing Summary</h4>
                  <div className="space-y-1 text-sm">
                    <p className="text-gray-400">
                      <span className="text-white">{selectedAmenities.length}</span> amenities selected
                    </p>
                    <p className="text-gray-400">
                      <span className="text-white">{images.length}</span> images added
                    </p>
                  </div>
                </div>
              </div>

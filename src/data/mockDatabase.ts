...
export interface User {
  ...
  isOnline: boolean;
  preferences?: UserPreferences | LandlordPreferences | {};
...
const generateUser = (role: UserRole, index: number): User => {
  ...
  return {
    ...
    role,
    tier: faker.helpers.arrayElement<SubscriptionTier>([
      "FREE",
      "PRO",
      "PREMIER",
    ]),
    verificationStatus: faker.helpers.arrayElement<VerificationStatus>([
      "VERIFIED",
      "UNVERIFIED",
      "PENDING",
      "REJECTED",
    ...
    preferences:
      role === "TENANT"
        ? {
            ...
            preferredLgas: faker.helpers.arrayElements(
              LAGOS_LGAS.map((l) => l.id),
              { min: 1, max: 5 },
            ),
            ...
            bedrooms: faker.helpers.arrayElements([1, 2, 3, 4, 5], {
              min: 1,
              max: 3,
            }),
            amenities: faker.helpers.arrayElements(
              ["PARKING", "SECURITY", "GYM", "POOL", "GENERATOR"],
              { min: 1, max: 4 },
            ),
          }
        : role === "LANDLORD"
          ? {
              ...
              propertySize: faker.helpers.arrayElement([
                "200k-500k",
                "500k-1m",
                "1m-5m",
                "5m+",
      ...
      ? [
          {
            ...
            status: faker.helpers.arrayElement<VerificationStatus>([
              "VERIFIED",
              "PENDING",
            ]),
...
const generateListing = (owners: User[], index: number): Listing => {
  ...
  return {
    ...
    status: faker.helpers.arrayElement<ListingStatus>([
      "ACTIVE",
      "PENDING",
      "APPROVED",
      "REJECTED",
...
// Users
const LANDLORDS = Array.from({ length: 25 }, (_, i) =>
  generateUser("LANDLORD", i),
);
...
const OFFICERS: Officer[] = [
  ...
  ...Array.from({ length: 5 }, (_, i) => ({
    ...
    assignedRegions: faker.helpers.arrayElements(
      LAGOS_REGIONS.map((r) => r.id),
      { min: 1, max: 3 },
    ),
    assignedLgas: faker.helpers.arrayElements(
      LAGOS_LGAS.map((l) => l.id),
      { min: 3, max: 8 },
    ),
...
const DISPUTES = Array.from({ length: 20 }, (_, i) =>
  generateDispute(ALL_USERS, i),
);

// Verification Requests
const VERIFICATION_REQUESTS: VerificationRequest[] = Array.from(
  { length: 15 },
  (_, i) => {
    ...
    return {
      ...
      status: faker.helpers.arrayElement<RequestStatus>([
        "PENDING",
        "APPROVED",
        "REJECTED",
      ]),
      ...
      lgaName: lga.name,
      propertyCount:
        user.role === "LANDLORD" ? faker.number.int({ min: 1, max: 10 }) : 0,
    };
  },
);

// Subscription Upgrades
const SUBSCRIPTION_UPGRADES: SubscriptionUpgrade[] = Array.from(
  { length: 12 },
  (_, i) => {
    const user = faker.helpers.arrayElement(
      ALL_USERS.filter((u) => u.tier !== "PREMIER"),
    );
    ...
    return {
      ...
      paymentReference: faker.datatype.boolean()
...
const ESCROW_ACCOUNTS: EscrowAccount[] = Array.from({ length: 25 }, (_, i) => {
  ...
  const listing = faker.helpers.arrayElement(
    LISTINGS.filter((l) => l.ownerId === landlord.id) || LISTINGS,
  );
  ...
  return {
    ...
    status: faker.helpers.arrayElement<EscrowStatus>([
      "PENDING",
      "HELD",
      "RELEASED",
      "REFUNDED",
...
const ACTIVITY_LOGS: ActivityLog[] = Array.from({ length: 200 }, (_, i) => {
  ...
  return {
    ...
    userName: `${user.firstName} ${user.lastName}`,
    userRole:
      "role" in user && user.role === "SUPER_ADMIN"
        ? "SUPER_ADMIN"
        : "role" in user
          ? user.role
          : "OFFICER",
...
const ERROR_LOGS: ErrorLog[] = Array.from({ length: 30 }, (_, i) => ({
  ...
  service: faker.helpers.arrayElement<"UG-Core" | "UG-App" | "UG-Officer">([
    "UG-Core",
    "UG-App",
    "UG-Officer",
  ]),
...
const FEATURE_FLAGS: FeatureFlag[] = [
  {
    ...
    isEnabled: true,
    enabledFor: ["TENANT", "LANDLORD", "AGENT"],
  ...
  },
  {
    ...
    key: "ESCROW_V2",
    name: "Escrow 2.0",
    description: "New escrow system with milestone payments",
    isEnabled: false,
    enabledFor: ["LANDLORD", "AGENT"],
    createdAt: "2024-01-01T00:00:00Z",
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    description: "Allow video uploads for listings",
    isEnabled: true,
    enabledFor: ["LANDLORD", "AGENT"],
    createdAt: "2024-01-01T00:00:00Z",
    updatedAt: new Date().toISOString(),
...
const TIER_CONFIGS: TierConfig[] = [
  {
    ...
    name: "Free",
    price: 0,
    features: ["5 Listing Views/day", "Basic Search", "Email Support"],
    listingLimit: 1,
    contactsPerMonth: 5,
...
const APP_CONFIGS: AppConfig[] = [
  ...
  {
    id: "config-002",
    key: "REQUIRE_2FA_OFFICERS",
    value: "true",
    category: "SECURITY",
    description: "Require 2FA for all officers",
    updatedAt: new Date().toISOString(),
    updatedBy: "officer-001",
  },
  {

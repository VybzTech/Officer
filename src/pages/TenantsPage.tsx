...
export function TenantsPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const users = await usersApi.getAll("TENANT");
      setData(users);
    } catch (error) {
  ...
  const getRandomEscrow = (userId: string) => {
    return userId.length % 4 === 0 ? "Active" : "None";
  };

  const getRiskFlag = (userId: string) => {
    return userId.length % 7 === 0 ? "High Risk" : "Clear";
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <BigHeader useHubot subtitle="Manage tenant profiles, verification status, and viewing preferences.">
            Tenant Management
          </BigHeader>
        </div>
          ...
          {isFilterDropdownOpen && (
            <div className="absolute right-0 top-[calc(100%+0.5rem)] w-56 bg-white border border-gray-100 shadow-xl rounded-2xl p-2 z-[100] animate-fade-in">
              ...
              {[
                ...
                { id: "basic", label: "Basic", className: "text-blue-500" },
                { id: "premium", label: "Premium", className: "text-purple-700 font-bold" },
                { id: "pro", label: "Pro", className: "text-amber-500 font-bold" },
              ].map((option) => (
                <button
                  key={`tier-${option.id}`}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredData.map((user) => {
            ...
            const riskFlag = getRiskFlag(user.id);
            ...
            return (
              <div
                key={user.id}
                className={cn(
                  "group relative bg-white rounded-[32px] border p-6 shadow-sm hover:shadow-md transition-all duration-300 pb-[76px]",
                  riskFlag === 'High Risk' ? "border-red-200" : "border-gray-100"
                )}
              >
                  ...
                  <div className="relative">
                    ...
                    {isMenuOpen && (
                      <div className="absolute right-0 top-12 w-48 bg-white border border-gray-100 shadow-xl rounded-2xl p-2 z-50 animate-fade-in">
                        <button onClick={() => handleAction('Identity Verification', user.firstName)} className="w-full text-left px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 rounded-xl flex items-center gap-2">
                          <Shield className="h-4 w-4 text-blue-500" /> Verify Identity
                        </button>
                        <button onClick={() => handleAction('Preferences Update', user.firstName)} className="w-full text-left px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 rounded-xl flex items-center gap-2">
                          <Settings2 className="h-4 w-4 text-gray-400" /> Preference Profile
                        </button>
                        <button onClick={() => handleAction('AI Matches', user.firstName)} className="w-full text-left px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 rounded-xl flex items-center gap-2">
                          <BrainCircuit className="h-4 w-4 text-purple-500" /> AI Match History
                        </button>
                        ...
                        <button onClick={() => handleAction('Tier Upgrade', user.firstName)} className="w-full text-left px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 rounded-xl flex items-center gap-2">
                          <ArrowUpCircle className="h-4 w-4 text-amber-500" /> Upgrade Tier
                        </button>

                        <div className="h-px bg-gray-100 my-1"></div>

                        <button onClick={() => handleAction('Risk Flagging', user.firstName)} className="w-full text-left px-3 py-2 text-sm font-bold text-danger hover:bg-danger/10 rounded-xl flex items-center gap-2">
                          <AlertOctagon className="h-4 w-4" /> Flag Risk
                        </button>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-6 pt-4 border-t border-gray-50">
                  <div className="p-3 bg-gray-50 rounded-2xl flex flex-col">
                    <p className="text-[10px] uppercase font-bold tracking-widest text-gray-500 flex items-center gap-1 mb-1"><AlertOctagon className="w-3 h-3" /> Risk Status</p>
                    <p className={cn("text-xs font-bold leading-none mt-1", riskFlag === 'High Risk' ? 'text-red-600' : 'text-green-600')}>{riskFlag}</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-2xl flex flex-col">
                    <p className="text-[10px] uppercase font-bold tracking-widest text-gray-500 flex items-center gap-1 mb-1"><Wallet className="w-3 h-3" /> Escrow</p>
                    <p className={cn("text-xs font-bold leading-none mt-1", escrowStatus === 'Active' ? 'text-blue-600' : 'text-gray-500')}>{escrowStatus}</p>
                  </div>
                </div>

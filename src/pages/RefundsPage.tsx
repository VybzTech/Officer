...
export function RefundsPage() {
  ...
  const { refundRequests, isLoading, fetchRefundRequests } = useDataStore();
  ...
  const filteredRequests = refundRequests.filter((request) => {
    const matchesSearch =
      ...
      request.disputeId.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 max-w-md">
          <Input
            placeholder="Search by user, reason, dispute ID..."
            icon={Search}
            value={searchQuery}
      ...
      {filteredRequests.length === 0 && (
        <Card padding="lg" className="text-center">
          <RotateCcw className="h-12 w-12 text-gray-300 mx-auto" />
